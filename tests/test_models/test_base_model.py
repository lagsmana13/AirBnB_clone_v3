...
'''
    All the examination for the base_model are implemented here.
'''
...
class TestBase(tests.unittest.TestCase):
    '''
        Exploring the base class model.
    '''
    def setUp(self):
        '''
    ...
    def test_ids_differ(self):
        '''
            Checks that the identities between two instances are different.
        '''
        new_model = BaseModel()
    ...
    def test_str_overide(self):
        '''
            Checks that the accurate communication gets printed.
        '''
        backup = sys.stdout
    ...
    @tests.unittest.skipIf(storage == "db", "Testing database storage only")
    def test_save(self):
        '''
    ...
    @tests.unittest.skipIf(storage != "db", "Testing if using DBStorage")
